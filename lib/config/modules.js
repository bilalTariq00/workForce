...
export function hasModuleAccess(user, moduleCode) {
  ...
  // First check role template permissions (if employee has roleTemplateId)
  // Role templates provide granular control and override role defaults
  if (user.roleTemplateId && user.roleTemplateId.permissions) {
    const hasPermission = user.roleTemplateId.permissions.some(
      p => p.module === moduleCode && p.actions && p.actions.length > 0
    );
    if (hasPermission) return true;
  }
  
  // Second: Check role-based default access (for all employees with roles)
  // This is the PRIMARY access method for HR-created employees
  // All employees created by HR have a role, so they get automatic access
  if (user.role) {
    const roleModules = ROLE_DEFAULT_MODULES[user.role] || [];
    if (roleModules.includes(moduleCode)) return true;
  }
  
  // Third: Fallback to purchased modules (for backward compatibility or edge cases)
  // This is mainly for users who manually purchased modules before role-based system
